//界面文字效果
$(function () {
    $(".header .header1").fadeIn(3000).fadeOut(3000)
})
setTimeout(function () {
    $(".header2").fadeIn(3000)
}, 6000)
...
layui.use('flow', function () {
    ...
    flow.load({
        ...
        , done: function (page, next) { //到达临界点（默认滚动触发），触发下一页
            ...
            $.get('article' + "?page=" + page, function (res) {
                //假设你的列表返回在data集合中
                var praise_bids = [];
                ...
                layui.each(res.data, function (index, item) {
                    let top = item.top == 1 ? "置顶" : '';
                    let cssName = item.top == 1 ? 'title-top' : 'title-empty';
